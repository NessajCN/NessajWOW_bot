...
from members import MemberStatistics
...
async def _message_handler(event, message: qqbot.Message):
    ...
    if message.content.startswith(config["prefix"]):
        ...
        embed_post = await member_stats.members
...
async def _message_reaction_handler(event, reaction: qqbot.Reaction):
    ...
    # 通过api发送回复消息
    await msg_api.post_message(reaction.channel_id, send)

if __name__ == "__main__":
    # async的异步接口的使用示例
    token = qqbot.Token(config["token"]["appid"], config["token"]["token"])
    guild_id = qqbot.UserAPI(token,False).me_guilds()[0].id
    member_stats = MemberStatistics(token,guild_id)
